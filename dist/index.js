 ...
 * @returns 処理済みのテキスト
 */
function processEmphasisInText(text, isListItemContent = false) {
    let result = text;
 ...
 */
export function fixMarkdown(markdown) {
    let result = markdown;
    // URL を含む全角括弧を半角括弧に変換し、前後にスペースを挿入
    // スペースの有無を正規化する
    result = result.replace(/\s*（\s*(https?:\/\/[^）]+)\s*）\s*/g, (match, url) => {
        // 前後のスペースを1つに正規化
        return ` (${url}) `;
    });
    ...
    result = result.replace(/(?<!\])\s*\(\s*(https?:\/\/[^)\s]+)\s*\)\s*/g, (match, url) => {
        // 前後のスペースを1つに正規化
        return ` (${url}) `;
    });
